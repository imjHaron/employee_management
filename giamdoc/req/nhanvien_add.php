...
if (isset($_POST['fname']) &&
    ...
    if (empty($fname)) {
        ...
        header("Location: ../nhanvien_add.php?error=$em&$data");
        exit;
    }else if (empty($lname)) {
        ...
        $sql  = "INSERT INTO
                 nhanviens(fname, lname, gioitinh, diachi, email, ngaysinh, ngayvaolam, sdt, cccd, phancong_id)
                 VALUES(?,?,?,?,?,?,?,?,?,?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$fname, $lname, $gioitinh, $diachi, $email, $ngaysinh, $ngayvaolam, $sdt, $cccd, $phancongs]);
        $sm = "Nhân viên mới được tạo thành công!";
        header("Location: ../nhanvien_add.php?success=$sm");
        exit;
    }
    
  }else {
    $em = "Có lỗi xảy ra";
    header("Location: ../nhanvien_add.php?error=$em");
    exit;
  }
